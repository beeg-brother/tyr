 ...
 */
    ...
    //int InputElement::numLines;
    //int InputElement::position;
    // TODO: implement horizontal scrolling

    InputElement::InputElement(int lines){
        // TODO: implement lines
        numLines = lines;
        position = 0;
    }

    void InputElement::onFocus(){
        isFocused = true;
        curs_set(1);
        return;
    }

    void InputElement::deFocus(){
        isFocused = false;
        curs_set(1);
        return;
    }

    int InputElement::requestNumLines(int width){
        // TODO: implement lines
        return 3;
    }
...
//            attroff(COLOR_PAIR(textColor));
    ...
    void InputElement::placeCursor(WINDOW* win, int startY){
        wmove(win, startY+1, std::min(getmaxx(win) - 2, position + 1));
    }

    void InputElement::handleInput(int c){
...
// otherwise, selected button(s) have a distinct highlight
    ...
    ButtonsElement::ButtonsElement(std::vector<std::string> opts, bool exitOnSelect){
    ...
    void ButtonsElement::onFocus(){
        isFocused = true;
        curs_set(0);
        return;
    }

    void ButtonsElement::deFocus(){
        isFocused = false;
        curs_set(1);
        return;
    }

    int ButtonsElement::requestNumLines(int width){
        // TODO: make this accurate
        return 3;
    }

    void ButtonsElement::refresh(WINDOW* win, int startY){
        // TODO: center buttons
    ...
    void ButtonsElement::placeCursor(WINDOW* win, int startY){
        return;
    }

    void ButtonsElement::handleInput(int c){
...
//const char* message;
    ...
    StringElement::StringElement(const char* str, int len){ // see https://www.oreilly.com/library/view/optimized-c/9781491922057/ch04.html for argument
        message = str;
        length = len;
    }
    ...
    void StringElement::onFocus(){
        isFocused = true;
        curs_set(0);
        return;
    }

    void StringElement::deFocus(){
        isFocused = false;
        curs_set(1);
        return;
    }

    int StringElement::requestNumLines(int width){
        // round up division. this prolly sucks tbh
        return length / width + (length % width != 0);
    }

    void StringElement::refresh(WINDOW* win, int yval){
        mvwaddstr(win, yval, 0, message);
    }

    void StringElement::placeCursor(WINDOW* win, int startY){
        return;
    }

    void StringElement::handleInput(int c){
        // do nothing
        return;
    }
...
*/
    ...
    Dialog::Dialog(std::vector<std::shared_ptr<DialogElement>> els){

        // assuming that we want the max width to be 1/3 of the screen (idk i just picked this #)
        int width = (getmaxx(stdscr) + 2)/3; // this looks jank but stack overflow says this will round the division up
        ...
        // center dialog box
        int start_x = (getmaxx(stdscr) + 1)/2 - (width + 1)/2; // again, rounding up the division
        int start_y = (getmaxy(stdscr) + 1)/2 - (height + 1)/2;

        // offsets to account for the border
        window::Window::create_windows(height + 2, width + 2, start_y - 1, start_x - 1, height, width, start_y, start_x);
