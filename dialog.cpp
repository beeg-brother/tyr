...
namespace dialog {
     ...
     * otherwise, selected button(s) ave a distinct highlight
     */
    ...
    //public:
        Dialog::Dialog(std::vector<std::shared_ptr<DialogElement>> els){
            ...
            int width = (getmaxx(win) + 2)/3; // this looks jank but stack overflow says this will round the division up
            ...
            // center dialog box
            int start_x = (getmaxx(win) + 1)/2 - (width + 1)/2; // again, rounding up the division
            int start_y = (getmaxy(win) + 1)/2 - (height + 1)/2;
